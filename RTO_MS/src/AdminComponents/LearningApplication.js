...
function GetSingleLearnerApplication() {
    ...
    useEffect(() => {
      const fetchApplicationDetails = async () => {
        try {
          const response = await fetch(`http://localhost:8080/admin/getLearnerApplication?applicationId=${applicationId}`);
          if (!response.ok) {
    ...
    const handleDownload = () => {
      ...
      if (selectedDocument) {
        ...
        fetch(`http://localhost:8080/admin/getDocuments?applicationId=${applicationId}&name=${selectedDocument}`)
    ...
    const handleApproval = () => {
        // Make PUT request to approve the application
        fetch(`http://localhost:8080/admin/learnerApplication/updateStatus?LearnerAppId=${applicationId}&status=APPROVED`, {
          method: 'PUT',
        })
        .then(response => {
          if (!response.ok) {
        ...
        .then(data => {
          ...
          history.push({
            ...
            state: { isSuccess: true, message: data.msg }
          });
        })
      ...
      const handleRejection = () => {
        // Make PUT request to reject the application
        fetch(`http://localhost:8080/admin/learnerApplication/updateStatus?LearnerAppId=${applicationId}&status=REJECTED`, {
          method: 'PUT',
        })
        .then(response => {
          if (!response.ok) {
